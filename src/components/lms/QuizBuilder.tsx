...
export function QuizBuilder({ quizId, initialQuestions }: QuizBuilderProps) {
  ...
  const [quizState, setQuizState] = useState<any>({
     ...
     ...initialQuestions // Needs to be merged properly from parent props ideally
  });
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
        const res = await saveQuestions(quizId, questions);
        toast.success('Quiz structure synchronized');
    } catch (e) {
    ...
    <div className="space-y-10 py-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 bg-[#0b0b14] border border-white/5 p-8 rounded-[40px] shadow-2xl relative overflow-hidden group">
         ...
         <div className="relative z-10">
            ...
            <p className="text-white/40 text-sm font-medium mt-1 italic">Build complex assessments with 10+ question types and AI feedback.</p>
         </div>
        ...
        <div className="lg:col-span-1 space-y-6">
           <div className="p-8 rounded-[40px] bg-[#0b0b14] border border-white/5 space-y-6 shadow-xl">
              ...
              <div className="grid grid-cols-1 gap-2">
                 ...
                 <ToolItem icon={FileText} label="Short Answer (Keywords)" onClick={() => addQuestion('short_answer')} />
              ...
              <div className="grid grid-cols-1 gap-2">
                 ...
                 <ToolItem icon={Layout} label="Ordering Sequence" onClick={() => addQuestion('ordering')} />
                 <ToolItem icon={Video} label="Video Response" onClick={() => addQuestion('video_response')} />
                 <ToolItem icon={HelpCircle} label="Scenario Context" onClick={() => addQuestion('scenario')} />
           ...
           <div className="p-8 rounded-[40px] bg-linear-to-br from-[#6c47ff] to-[#8b5cf6] shadow-2xl relative overflow-hidden group">
              ...
              <p className="text-white/70 text-[11px] leading-relaxed relative z-10">
                Automatically generate 5 challenging questions based on your course content with one click.
              </p>
              <Button className="w-full mt-6 bg-white text-[#6c47ff] font-black italic uppercase text-[10px] rounded-xl h-10 tracking-widest gap-2">
                  ...
                  <div className="p-8 space-y-6">
                     ...
                     {q.type === 'short_answer' && (
                        <div className="space-y-4">
                           ...
                           <Input 
                              placeholder="e.g. trigger, automation, workflow"
                           ...
                           />
                           <p className="text-[10px] text-white/20 italic italic">The system checks if the student's answer contains these keywords.</p>
                        </div>
                     )}
                     ...
                     {q.type === 'scenario' && (
                        <div className="space-y-4">
                           <span className="text-[10px] font-black text-[#6c47ff] uppercase tracking-widest">Scenario Background Description</span>
                           <textarea 
                              placeholder="Present your multi-paragraph business case or problem here..."
                     ...
                     {/* Essay/Short Answer Placeholder */}
                     {(q.type === 'essay' || q.type === 'video_response') && (
                        <div className="p-12 border border-dashed border-white/5 rounded-[32px] text-center bg-white/[0.01]">
                           <span className="text-[10px] font-black text-white/10 uppercase tracking-[0.3em]">Manual Grading Required For This Type</span>
                        </div>
                     )}
                  </div>
               </CardContent>
